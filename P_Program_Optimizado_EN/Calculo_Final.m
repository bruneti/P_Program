...
% eigen_vector1 = spin up, eigen_vector2 = spin down.
 C_mu=eigen_vector1(:,320);   % >>> initial state index
 C_mup=eigen_vector1(:,321);  % >>> final state index
else
...
Gamma_R=4*constants.q^2*DE^3*(abs(Mu))^2/(3*pi*eps_0*hbar^4*c^3); % Radiative transition rate (same reference as Mu)
